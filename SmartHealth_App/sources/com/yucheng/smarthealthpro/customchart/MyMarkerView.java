...
public class MyMarkerView extends MarkerView {
    ...
    public MyMarkerView(Context context, int layoutResource, List<TemperBean> mTemperBean, List<BloodPressureBean> mBloodPressureBean, List<SleepDayInfo> mSleepInfo, List<StepBean> mStepBean) {
    ...
    public MyMarkerView(Context context, int layoutResource, List<TemperBean> mTemperBean, List<BloodPressureBean> mBloodPressureBean, List<SleepDayInfo> mSleepInfo, List<StepBean> mStepBean, List<PhyDayInfo> mPhyInfo) {
        super(context, layoutResource);
        this.DEFAULT_INDICATOR_COLOR = -3218945;
        this.ARROW_HEIGHT = dp2px(10);
        this.ARROW_WIDTH = dp2px(15);
    ...
    public void refreshContentDayKetone(Entry e2, Highlight highlight, List<TemperBean> temperBeans) {
        if (e2 instanceof CandleEntry) {
            ...
            float y = e2.getY();
            String str = y + "";
            this.tvContent.setText(String.format(TimeModel.ZERO_LEADING_NUMBER_FORMAT, Integer.valueOf(((int) e2.getX()) / 60)) + ":" + String.format(TimeModel.ZERO_LEADING_NUMBER_FORMAT, Integer.valueOf(((int) e2.getX()) % 60)) + StringUtils.SPACE + FormatUtil.keep1(y) + this.mUricAcitUnit);
        }
        super.refreshContentDayUricAcid(e2, highlight, temperBeans);
    }
            ...
            if (temperBeans != null) {
                for (int i2 = 0; i2 < temperBeans.size(); i2++) {
                    if (i2 == ((int) e2.getX())) {
                        float y = e2.getY();
                        String str = y + "";
                        this.tvContent.setText(temperBeans.get(i2).getmMonthDay() + StringUtils.SPACE + FormatUtil.keep1(y) + this.mUricAcitUnit);
                    }
                }
            }
        }
    ...
    public void refreshContentDayHeartRate(Entry e2, Highlight highlight, List<TemperBean> temperBeans) {
        ...
        if (e2 instanceof CandleEntry) {
            ...
            this.tvContent.setText(String.format(TimeModel.ZERO_LEADING_NUMBER_FORMAT, Integer.valueOf(((int) e2.getX()) / 60)) + ":" + String.format(TimeModel.ZERO_LEADING_NUMBER_FORMAT, Integer.valueOf(((int) e2.getX()) % 60)) + StringUtils.SPACE + bigDecimal.setScale(0, 4).toString() + " bpm");
        }
        super.refreshContentTemp(e2, highlight, temperBeans);
    }
    ...
    public void refreshContentDayBloodPressure(Entry e2, Highlight highlight, List<TemperBean> temperBeans) {
        if (e2 instanceof CandleEntry) {
            ...
            new BigDecimal(candleEntry.getOpen());
            new BigDecimal(candleEntry.getClose());
            candleEntry.getmTime();
            Log.d("getTimeStr", "getTimeStr: " + candleEntry.getTimeStr());
            this.tvContent.setText(candleEntry.getTimeStr());
        } else {
    ...
    public void refreshContentStep(Entry e2, Highlight highlight, List<StepBean> mStepBean) throws Resources.NotFoundException {
        if (e2 instanceof CandleEntry) {
            ...
            for (int i2 = 0; i2 < mStepBean.size(); i2++) {
                if (i2 == ((int) e2.getX())) {
                    StepBean stepBean = mStepBean.get(i2);
                    String string = stepBean.unit;
                    if (string == null) {
                        string = getResources().getString(R.string.step_unit);
                    }
                    this.tvContent.setText(stepBean.getBeginhhmm() + StringUtils.SPACE + ((int) e2.getY()) + string);
                }
            }
        }
    ...
    public void refreshContentDayPressure(Entry e2, Highlight highlight, List<TemperBean> temperBeans) {
        ...
        if (e2 instanceof CandleEntry) {
            ...
            this.tvContent.setText(String.format(TimeModel.ZERO_LEADING_NUMBER_FORMAT, Integer.valueOf((int) e2.getX())) + ":00 " + ((int) e2.getY()) + StringUtils.SPACE);
        }
        super.refreshContentTemp(e2, highlight, temperBeans);
    }
    ...
    public void refreshContentWeekMonthPressure(Entry e2, Highlight highlight, List<TemperBean> temperBeans) {
        ...
        if (e2 instanceof CandleEntry) {
            ...
            for (int i2 = 0; i2 < temperBeans.size(); i2++) {
                if (i2 == ((int) e2.getX())) {
                    this.tvContent.setText(temperBeans.get(i2).getmMonthDay() + StringUtils.SPACE + ((int) e2.getY()) + StringUtils.SPACE);
                }
            }
        }
        super.refreshContentTemp(e2, highlight, temperBeans);
